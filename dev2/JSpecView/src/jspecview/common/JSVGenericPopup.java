...
abstract public class JSVGenericPopup implements JmolPopupInterface,
    JSVAbstractMenu {
  ...
  public String jpiGetMenuAsString(String title) {
    ...
    return (new JSVPopupResourceBundle())
        .getMenuAsText(title);
  }
  ...
  private void addMenuItems(String parentId, String key, Object menu,
    ...
    while (st.hasMoreTokens()) {
      ...
      if (label.equals("null")) {
        ...
        newMenu = menuCreateCheckboxItem(menu, label, basename,
        ...
        if (!isJS && item.contains("JS"))
          continue;
        newMenu = menuCreateItem(menu, label, item, id + "." + item);
      }
      ...
      // menus or menu items:
      if (item.indexOf("NOMULTI") >= 0) {
        SingleModelOnly.addLast(newMenu);
      } else if (item.indexOf("MULTI") >= 0) {
