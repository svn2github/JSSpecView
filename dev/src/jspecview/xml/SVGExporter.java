...
public class SVGExporter {
  ...
  public static void exportAsSVG(String fileName, Graph graph) throws IOException{
    FileWriter writer;
    writer = new FileWriter(fileName);

    exportAsSVG(writer, graph.getXYCoords(), "", 0, graph.getNumberOfPoints() -1,
   ...
   * @throws IOException
   */
  public static void exportAsSVG(Writer writer, Graph graph) throws IOException{
    exportAsSVG(writer, graph.getXYCoords(), "", 0, graph.getNumberOfPoints() -1,
                graph.getXUnits(), graph.getYUnits(),
                graph.isContinuous(), graph.isIncreasing(),
                Color.lightGray, Color.white, Color.black,
  ...
  /**
   * Exports a Graph as SVG and writes it to a String
   * @param graph the graph
   * @return the SVG text as a String
   */

  public static String exportAsSVG(Graph graph){
  ...
  /**
   * Exports the Graphs that are displayed by JSVPanel to a Writer with the same
   * Color Scheme as the JSVPanel
   * @param writer the writer
   * @param jsvp the JSVPanel
   * @throws IOException
   */
  public static void exportAsSVG(Writer writer, JSVPanel jsvp) throws IOException{
    int numOfSpectra = jsvp.getNumberOfSpectra();

    if(numOfSpectra == 1){

      Graph graph = (Graph)jsvp.getSpectrumAt(0);

      exportAsSVG(writer, graph.getXYCoords(), graph.getTitle(),
                  0, graph.getNumberOfPoints() -1,
                  graph.getXUnits(), graph.getYUnits(),
                  graph.isContinuous(), graph.isIncreasing(),
      ...
      Coordinate[][] coordList = jsvp.getXYCoordsList();

      for(int i = 0; i < numOfSpectra; i++){
        startIndices[i] = 0;
        endIndices[i] = coordList[i].length - 1;
      }

      exportAsSVG(writer, coordList , " ",
   ...
   * with the same Color Scheme as the JSVPanel
   * @param fileName the name of the file
   * @param jsvp the JSVPanel
   * @throws IOException
   */
  public static void exportAsSVG(String fileName, JSVPanel jsvp) throws IOException {
    FileWriter writer;
    writer = new FileWriter(fileName);

    exportAsSVG(writer, jsvp);
  }


  /**
   * Exports the Graphs that are displayed by JSVPanel to a writer,
   ...
   * @param doZoom if true the zoomed view is exported, otherwise full graph is
   * exported
   * @throws IOException
   */
  public static void exportAsSVG(Writer writer,  JSVPanel jsvp, int specIndex, boolean doZoom) throws IOException{
  ...
  public static void exportAsSVG(String fileName,  JSVPanel jsvp, int specIndex,
    ...
    FileWriter writer;
    writer = new FileWriter(fileName);
    exportAsSVG(writer, jsvp, specIndex, doZoom);
  }

  public static void exportAsSVG(Writer writer, Vector graphs){
    int numOfSpectra = graphs.size();
    ...
    if(numOfSpectra == 1){
      ...
      Coordinate[][] coordList = new Coordinate[numOfSpectra][];

      for (int i = 0; i < coordList.length; i++) {
        coordList[i] = ((Graph)graphs.elementAt(i)).getXYCoords();
      }
   ...
   */
  public static void exportAsSVG(Writer writer, Coordinate[] xyCoords, String title,
    ...
    //Grid
    Vector<HashMap> vertGridCoords = new Vector<HashMap>();
    Vector<HashMap> horizGridCoords = new Vector<HashMap>();
    ...
    Vector<HashMap> xScaleList = new Vector<HashMap>();
    Vector<HashMap> xScaleListReversed = new Vector<HashMap>();
    Vector<HashMap> yScaleList = new Vector<HashMap>();
  ...
  public static void exportAsSVG(Writer writer, Coordinate[][] xyCoordsList, String title,
    ...
    Vector<Vector> newXYCoordsList = new Vector<Vector>();
    Vector<Coordinate> coords = new Vector<Coordinate>();
    for(int i = 0; i < xyCoordsList.length; i++){
      for(int j = startDataPointIndices[i]; j <= endDataPointIndices[i]; j++){
        Coordinate pt;
        pt = xyCoordsList[i][j].copy();
        coords.addElement(pt);
      }
      newXYCoordsList.addElement((Vector)coords.clone());
    }
