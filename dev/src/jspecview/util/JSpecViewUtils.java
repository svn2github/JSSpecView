...
import jspecview.common.IntegrateDialog;
...
public class JSpecViewUtils {
   ...
   * {@link jspecview.util.JSpecViewUtils.MultiScaleData} is generated with the
   * {@link jspecview.util.JSpecViewUtils#generateScaleData(jspecview.util.Coordinate[][],
  ...
  /**
    ...
    public static JSVPanel integrate(Frame owner, JInternalFrame frame, boolean showDialog){
      ...
      else if(integrateOn = hasIntegration(jsvp)){
  ...
  public static boolean isHNMR(JDXSpectrum spectrum) {
    ...
    HashMap headerTable = spectrum.getHeaderTable();
  ...
  public static JSVPanel removeIntegration(Frame mainframe, JInternalFrame frame, boolean showDialog) {
    if (frame == null)
      return null;
    JSVPanel jsvp = (JSVPanel) frame.getContentPane().getComponent(0);
    if (JSpecViewUtils.hasIntegration(jsvp)) {
      try {
        ...
        ((MainFrame)mainframe).writeStatus(ex.getMessage());
      }
    }
    return (JSVPanel)frame.getContentPane().getComponent(0);
  }
  ...
  /**
   * Copies an array of Coordinates to another array
   * @param xyCoords the array of Coordinates
   * @return the new array of Coordinates
   */
  public static Coordinate[] copyCoordinateArray(Coordinate[] xyCoords){
    Coordinate[] newArray = new Coordinate[xyCoords.length];
    for(int i = 0; i < xyCoords.length; i++){
      newArray[i] = xyCoords[i].copy();
    }
    return newArray;
  }
   ...
   * overlayed. An instance of {@link jspecview.util.JSpecViewUtils.MultiScaleData}
   * may be generated by {@link jspecview.util.JSpecViewUtils#generateScaleData(
   * jspecview.util.Coordinate[][], int[], int[], int, int)}
   * You do not need to create an instance of this
   * class manually. Instead call the
   * {@link jspecview.util.JSpecViewUtils#generateScaleData(
   * jspecview.util.Coordinate[], int, int, int, int)}.
   */
